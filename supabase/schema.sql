...
create table if not exists public.profiles (
  ...
  work_level_paid integer not null default 1,
...
create table if not exists public.tasks (
  ...
  deleted_at timestamptz,
  -- Coluna do Kanban da comunidade — só cobre os estados "em aberto"
  -- (backlog/a fazer/em andamento/aguardando aprovação); Bloqueada e
  -- Concluída são derivadas de "blocked"/"completed" acima, não daqui, pra
  -- não duplicar fonte de verdade. board_order ordena os cards dentro de
  -- uma coluna (arrastar e soltar).
  board_status text not null default 'todo' check (board_status in ('backlog', 'todo', 'in_progress', 'awaiting_approval')),
...
-- Nível do personagem (= nível de trabalho): mesma curva do client (src/utils/level.ts),
-- portada pro banco pra calcular de forma confiável quando premiar moedas.
...
-- Cosméticos equipados (moldura, círculo de luz, paleta, pet) + nível de
-- trabalho + nível do pet de vários usuários de uma vez, pra qualquer
-- usuário autenticado poder ver a personalização completa de todo mundo no
-- tabuleiro 3D (antes só dava pra ver a própria).
