-- Ao atribuir responsável por tarefa/subtarefa, os pontos/XP devem ir pra quem
-- efetivamente fez aquele pedaço do trabalho, não só para o dono da tarefa.
-- Decompõe a pontuação em duas partes:
--   - "base" da tarefa (1 unidade × complexidade): sempre credita o responsável
--     pela tarefa (tasks.user_id, que já reflete reatribuições via assign_task).
--   - cada subtarefa (1 unidade × complexidade): credita o responsável daquela
--     subtarefa (subtasks.assignee_id), caindo pro dono da tarefa se a
--     subtarefa não tiver responsável específico.
-- lost_points (penalidade por urgência ao expirar) continua inteira do dono da
-- tarefa, já que é sobre ter perdido o prazo como um todo, não sobre quem fez
-- o quê.
