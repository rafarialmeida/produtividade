  ...
  const ranking = useMemo(() => {
    return users
      .map((u) => {
        ...
        const communityCount = communities.filter((c) => c.memberIds.includes(u.id)).length
        return { user: u, lostPoints, expiredCount: expiredTasks.length, communityCount }
      })
      .filter((entry) => entry.communityCount > 0)
      .sort((a, b) => b.lostPoints - a.lostPoints)
  }, [users, tasks, communities])
